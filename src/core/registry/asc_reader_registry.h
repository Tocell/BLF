...
public:
    ...
    static inline AscLineKey asc_extract_key(const std::string& line)
    {
        ...
        // LIN: <Time> LIN ...
        if (t[1] == "LIN") return AscLineKey::Lin;

        // FlexRay: <Time> FLEXRAY ...
        if (t[1] == "FLEXRAY") return AscLineKey::FlexRay;

        // Ethernet: <Time> ETHERNET ...
        if (t[1] == "ETHERNET") return AscLineKey::Ethernet;

        // Error frame: <Time> <Ch> ErrorFrame
        if (t.size() >= 3 && t[2] == "ErrorFrame") return AscLineKey::CanError;
