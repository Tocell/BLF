...
void AscLogger::writer_header()
{
	...
	// �?start_measure_time_ 生成 header �?date 行（而不�?system_clock::now()�?
...
void AscLogger::writer_thread_handler()
{
	...
	while (is_running_.load())
	{
		...
		while (!msg_queue.empty())
		{
			...
			const auto estimated_write_size = w->estimate_write_size(*msg);
			if ((file_writer_.get_pos() + estimated_write_size) >= BUFFER_MAX_SIZE)
			{
				file_writer_.write(file_writer_.get_buffer(), file_writer_.get_pos());
				file_writer_.set_pos(0);
			}
