#include "can_driver_sync_message_blf_writer.h"
...
static WriterRegistrar<CanDriverSyncMessageBlfWriter> registrar(FileFormat::BLF, BusType::CAN_DRIVER_SYNC);

CanDriverSyncMessageBlfWriter::CanDriverSyncMessageBlfWriter()
{
	header_base_.signature = BL_OBJ_SIGNATURE;
	header_base_.header_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader);
	header_base_.object_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader) + sizeof(CanDriverSyncFrame);
	header_base_.object_type = BL_OBJ_TYPE_CAN_DRIVER_SYNC;
...
void CanDriverSyncMessageBlfWriter::set_timestamp_unit(TimeStampUnit unit)
{
	header_.time_flags = static_cast<uint32_t>(unit);
}

bool CanDriverSyncMessageBlfWriter::write(const BusMessage& msg, FileWriter& writer)
{
	const auto& can_msg = dynamic_cast<const CanDriverSyncMessage&>(msg);
	...
	writer.append(reinterpret_cast<const uint8_t*>(&can_frame), sizeof(CanDriverSyncFrame));
