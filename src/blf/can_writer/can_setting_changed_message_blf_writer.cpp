#include "can_setting_changed_message_blf_writer.h"
...
static WriterRegistrar<CanSettingChangedMessageBlfWriter> registrar(FileFormat::BLF, BusType::CAN_SETTING_CHANGE);

CanSettingChangedMessageBlfWriter::CanSettingChangedMessageBlfWriter()
{
	header_base_.signature = BL_OBJ_SIGNATURE;
	header_base_.header_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader);
	header_base_.object_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader) + sizeof(CanSettingChangedFrame);
	header_base_.object_type = BL_OBJ_TYPE_CAN_SETTING_CHANGED;
...
void CanSettingChangedMessageBlfWriter::set_timestamp_unit(TimeStampUnit unit)
{
	header_.time_flags = static_cast<uint32_t>(unit);
}

bool CanSettingChangedMessageBlfWriter::write(const BusMessage& msg, FileWriter& writer)
{
	const auto& can_msg = dynamic_cast<const CanSettigChangedMessage&>(msg);
	...
	writer.append(reinterpret_cast<const uint8_t*>(&can_frame), sizeof(CanSettingChangedFrame));
