#include "can_err_message_blf_writer.h"
...
static WriterRegistrar<CanErrMessageBlfWriter> registrar(FileFormat::BLF, BusType::CAN_ERR);

CanErrMessageBlfWriter::CanErrMessageBlfWriter()
{
    header_base_.signature = BL_OBJ_SIGNATURE;
    header_base_.header_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader);
    header_base_.object_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader) + sizeof(CanErrFrame);
    header_base_.object_type = BL_OBJ_TYPE_CAN_ERROR;
...
bool CanErrMessageBlfWriter::write(const BusMessage& msg, FileWriter& writer)
{
    ...
    const CanErrFrame& can_frame = can_msg.get_frame();
    ...
    writer.append(reinterpret_cast<const uint8_t*>(&can_frame), sizeof(CanErrFrame));
