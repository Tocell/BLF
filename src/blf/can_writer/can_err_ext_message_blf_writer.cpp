#include "can_err_ext_message_blf_writer.h"
...
static WriterRegistrar<CanErrExtMessageBlfWriter> registrar(FileFormat::BLF, BusType::CAN_ERR_EXT);

CanErrExtMessageBlfWriter::CanErrExtMessageBlfWriter()
{
	header_base_.signature = BL_OBJ_SIGNATURE;
	header_base_.header_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader);
	header_base_.object_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader) + sizeof(CanErrExtFrame);
	header_base_.object_type = BL_OBJ_TYPE_CAN_ERROR_EXT;
...
void CanErrExtMessageBlfWriter::set_timestamp_unit(TimeStampUnit unit)
{
	header_.time_flags = static_cast<uint32_t>(unit);
}

bool CanErrExtMessageBlfWriter::write(const BusMessage& msg, FileWriter& writer)
{
	const auto& can_msg = dynamic_cast<const CanErrExtMessage&>(msg);
	...
	writer.append(reinterpret_cast<const uint8_t*>(&can_frame), sizeof(CanErrExtFrame));
