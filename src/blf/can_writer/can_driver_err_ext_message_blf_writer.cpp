#include "can_driver_err_ext_message_blf_writer.h"
...
static WriterRegistrar<CanDriverErrExtMessageBlfWriter> registrar(FileFormat::BLF, BusType::CAN_DRIVER_ERR_EXT);

CanDriverErrExtMessageBlfWriter::CanDriverErrExtMessageBlfWriter()
{
	header_base_.signature = BL_OBJ_SIGNATURE;
	header_base_.header_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader);
	header_base_.object_size = sizeof(ObjectHeaderBase) + sizeof(ObjectHeader) + sizeof(CanDriverErrExtFrame);
	header_base_.object_type = BL_OBJ_TYPE_CAN_DRIVER_ERROR_EXT;
...
{
	const auto& can_msg = dynamic_cast<const CanDriverErrExtMessage&>(msg);
	...
	writer.append(reinterpret_cast<const uint8_t*>(&can_frame), sizeof(CanDriverErrExtFrame));
